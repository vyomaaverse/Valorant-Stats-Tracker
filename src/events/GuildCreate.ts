...
export class GuildCreate implements EventInterface {
  name = Events.GuildCreate;
  async execute(client: ClientInterface, ...args: any[]) {
    try {
          ...
          .then((channel) => {
            const embed = createEmbed({
              ...
              author: {
                ...
                name: `${client.user?.username} | Guild Joined`,
              },
              thumbnail: {
      ...
      client.logger.log("info", `Guild Joined ${guild.name} (${guild.id})`);
    } catch (error) {
